...
class PropertyController: UserController {
    ...
    static func savePropertyImagesToCoreDataAndFirebase(images: [UIImage], landlord: Landlord, forProperty property: Property, completion: @escaping (_ imageURL: String) -> Void) {
        ...
        for fullSizedImage in images {
            group.enter()
            let image = fullSizedImage.resizeImage(longestEdge: 480)
            FirebaseController.store(profileImage: image, forUserID: landlordID, and: property, completion: { (metadata, error, imageData) in
                if let error = error {
                    print(error.localizedDescription)
